...
export function VideoComparisonSection({ fileName, onFile }: Props) {
  ...
  const { selectedFile, videoUrls, timestamp, data } = useAnalysis();
    ...
    <section className="mx-auto mt-10 max-w-7xl px-6">
      ...
      <div className="grid gap-5 lg:grid-cols-2">
          ...
          <div className="pitch-bg relative flex aspect-video w-full items-center justify-center overflow-hidden rounded-xl border border-border">
            ...
            {selectedFile ? (
              ...
              <div className="flex flex-col items-center justify-center text-center">
                <PlayCircle className="h-16 w-16 text-white/80 drop-shadow-lg mb-2" />
                <span className="text-xs text-muted-foreground">Demo Video Loaded</span>
              </div>
            ) : (
        ...
        <div className="glass rounded-2xl p-4 flex flex-col">
          ...
          <div className="pitch-bg relative aspect-video w-full overflow-hidden rounded-xl border border-border flex-1 flex items-center justify-center">
            {viewMode === "overlay" && (
              videoUrls ? (
                <video
                  src={`http://localhost:8000${videoUrls.main}?t=${timestamp}`}
                  controls
                  autoPlay
                  loop
                  muted
            ...
            {viewMode === "pitch" && (
              videoUrls ? (
                <video
                  src={`http://localhost:8000${videoUrls.minimap}?t=${timestamp}`}
                  controls
                  autoPlay
                  loop
                  muted
              ...
              <div className="absolute inset-0 bg-background/95 p-6 overflow-auto text-sm">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between border-b border-border pb-2">
                    ...
                    <span className="font-medium">{data.feed.length}</span>
                  </div>
          ...
          <div className="mt-3 grid grid-cols-3 gap-2 text-center text-xs">
            <div className="rounded-lg bg-secondary/60 p-2"><div className="text-muted-foreground">Frames</div><div className="font-semibold tabular-nums">{videoUrls ? "~300" : "3,214"}</div></div>
            <div className="rounded-lg bg-secondary/60 p-2"><div className="text-muted-foreground">Tracked</div><div className="font-semibold tabular-nums">{data.players.length} players</div></div>
